...
function setScoreSingle(Poule $poule, int $homePlaceNr, int $awayPlaceNr, int $homeGoals, int $awayGoals, int $state = null) {
    ...
    $foundGames = $poule->getGames()->filter( function( Game $game ) use ($homePlace, $awayPlace) {
        $homePlaces = $game->getPlaces(Game::HOME)->map( function( GamPlace $gamePlace) { return $gamePlace->getPlace(); });
        $awayPlaces = $game->getPlaces(Game::AWAY)->map( function( GamPlace $gamePlace) { return $gamePlace->getPlace(); });

        $homePlacesHasHomePlace = $homePlaces->filter( function( $homePlaceIt ) use ($homePlace) { return $homePlaceIt === $homePlace; })->count() > 0;
        $homePlacesHasAwayPlace = $homePlaces->filter( function( $homePlaceIt ) use ($awayPlace) { return $homePlaceIt === $awayPlace; })->count() > 0;
        $awayPlacesHasHomePlace = $awayPlaces->filter( function( $awayPlaceIt ) use ($homePlace) { return $awayPlaceIt === $homePlace; })->count() > 0;
        $awayPlacesHasAwayPlace = $awayPlaces->filter( function( $awayPlaceIt ) use ($awayPlace) { return $awayPlaceIt === $awayPlace; })->count() > 0;
