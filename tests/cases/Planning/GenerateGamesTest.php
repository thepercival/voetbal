...
class GenerateGamesTest extends \PHPUnit\Framework\TestCase
{
    ...
    protected function assertSameGame( array $gameRounds, int $roundNr, int $subNr, array $home, array $away) {
        ...
        $this->assertSame( array_map( function( $poulePlace ) { return $poulePlace->getNumber(); }, $combination->getHome() ), $home);
        $this->assertSame( array_map( function( $poulePlace ) { return $poulePlace->getNumber(); }, $combination->getAway() ), $away);
    }
}
